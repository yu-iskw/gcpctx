...
"""Pure .gcpctx.toml parsing and validation (no filesystem I/O)."""
...
from gcpctx.core.model import GcpctxConfig, ProfileConfig
...
def parse_and_validate(raw: bytes, policy: SecurityPolicy) -> GcpctxConfig:
    """Parse and validate configuration from raw TOML bytes (policy required)."""
    parsed = tomllib.loads(raw.decode("utf-8"))
    _validate_raw(parsed, policy)
