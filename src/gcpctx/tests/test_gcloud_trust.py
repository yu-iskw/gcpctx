...
def test_resolve_gcloud_path_falls_back_when_stale_pin(
    ...
    monkeypatch.setattr(
        "gcpctx.gcloud_trust._configured_gcloud_path",
        lambda _cwd: STALE_PIN,
    )
    gcloud_script = fake_gcloud.parent / "gcloud"
    assert resolve_gcloud_path(project_tree) == str(gcloud_script)
...
def test_resolve_gcloud_path_fails_when_stale_pin_and_no_path(
    ...
    with pytest.raises(GcloudNotFoundError, match=f"pinned gcloud_path {STALE_PIN!r} not found"):
        resolve_gcloud_path(project_tree)
...
def test_resolve_gcloud_path_uses_first_on_path(
    tmp_path: Path,
    project_tree: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _write_executable_gcloud(fake_gcloud)
    monkeypatch.setenv("PATH", str(bin_dir))

    assert resolve_gcloud_path(project_tree) == str(fake_gcloud)
