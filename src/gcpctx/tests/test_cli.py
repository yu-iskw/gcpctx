...
def test_list_not_found(tmp_path: Path) -> None:
    result = runner.invoke(app, ["list", "--cwd", str(tmp_path)])
    assert result.exit_code == CONFIG_NOT_FOUND_EXIT


def test_hook_noop_outside_project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("GCPCTX_ACTIVE", raising=False)
    result = runner.invoke(app, ["hook", "--shell", "zsh", "--cwd", str(tmp_path)])
    assert result.exit_code == 0
    assert result.stdout == ""


def test_hook_deactivates_outside_project_when_active(
    tmp_path: Path,
...
def test_hook_activation_failure_emits_deactivate(
    project_tree: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(project_tree)
    result = runner.invoke(app, ["hook", "--shell", "zsh"])
    assert result.exit_code == 3
    assert "unset GCPCTX_ACTIVE" in result.stdout
    assert "approval required" in result.stderr.lower()


def test_config_set_and_unset_gcloud_path(
    ...
    set_result = runner.invoke(app, ["config", str(gcloud_bin)])
...
def test_activate_with_profile_flag_requires_approval(
    project_tree: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(project_tree)
    result = runner.invoke(app, ["activate", "--profile", "dev", "--shell", "zsh"])
    assert result.exit_code == 3
    assert "approval required" in result.stderr.lower()


def test_config_show_legacy_hint(project_tree: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(project_tree)
    result = runner.invoke(app, ["config", "show"])
    assert result.exit_code == 2
    assert "config show was removed" in result.stderr


def test_config_show_unset(project_tree: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(project_tree)
    result = runner.invoke(app, ["config"])
    assert result.exit_code == 0
    assert "gcloud_path" in result.stdout


def test_reset_removes_context_without_active_env(
    project_tree: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(project_tree)
    monkeypatch.delenv("GCPCTX_CONTEXT_ID", raising=False)
    ctx = resolve_project_context(project_tree)
    ctx_dir = paths.context_dir(ctx.context_id())
    ctx_dir.mkdir(parents=True)

    result = runner.invoke(app, ["reset", "--cwd", str(project_tree)])
...
def test_activate_with_profile_requires_approval(
    project_tree: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(project_tree)
    result = runner.invoke(app, ["activate", "dev", "--shell", "zsh"])
    assert result.exit_code == 3
    assert "approval required" in result.stderr.lower()
...
def test_install_prints_snippet_and_instructions(shell: str, rc_file: str) -> None:
    result = runner.invoke(app, ["install", "--shell", shell])
    assert result.exit_code == 0
    assert "# >>> gcpctx hook >>>" in result.stdout
    assert "gcpctx-switch" in result.stdout
